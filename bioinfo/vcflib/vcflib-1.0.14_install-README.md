...
Structure creating script (makeroom_vcflib_1.0.14.sh) moved to /sw/bioinfo/vcflib/makeroom_1.0.14.sh

LOG
---

We want vcflib/1.0.14 to choose to use its own contrib for tabixpp, though using an external one would be very nice.

We also have to force its RPATH to be reasonable. It unsets RPATH for some executables and leaves others incomplete.

    makeroom.sh -f -t vcflib -v 1.0.14 -w https://github.com/vcflib/vcflib -d "C++ library and cmdline tools for parsing and manipulating VCF files with python and zig bindings" -l MIT 
    ./makeroom_vcflib_1.0.14.sh 
    source /sw/bioinfo/vcflib/SOURCEME_vcflib_1.0.14
    cd $SRCDIR
...
We might also have to set the RPATH for the python pyvcflib in $SRCDIR/vcflib/build/pyvcflib.cpython-312-x86_64-linux-gnu.so
...
Make sure the pyvcflib cython library has RPATH set correctly.
    ...
    ml patchelf
    cd lib64
    patchelf --print-rpath pyvcflib.cpython-312-x86_64-linux-gnu.so
    ldd pyvcflib.cpython-312-x86_64-linux-gnu.so 
...
---
    ...
    /home/douglas/bin/makeroom.sh "-f" "-t" "vcflib" "-v" "1.0.14" "-w" "https://github.com/vcflib/vcflib" "-d" "C++ library and cmdline tools for parsing and manipulating VCF files with python and zig bindings" "-l" "MIT"
    ./makeroom_vcflib_1.0.14.sh
