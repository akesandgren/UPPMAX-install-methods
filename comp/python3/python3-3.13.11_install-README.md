...
Structure creating script (makeroom_python3_3.13.11.sh) moved to /sw/comp/python3/makeroom_3.13.11.sh
...
---
    ...
    makeroom.sh "-f" "-t" "python3" "-v" "3.13.11" "-c" "comp" "-w" "https://python.org/" "-l" "PSF License Agreement" "-L" "https://docs.python.org/3/license.html" "-d" "Python programming language plus a handful of widely used packages ; this module does not conflict with python/x.y.z modules"
    ./makeroom_python3_3.13.11.sh

    VERSION=3.13.11
    source /sw/comp/python3/SOURCEME_python3_${VERSION}
    cd $SRCDIR
...
Make sure the mf is correct. makeroom.sh can make a poor choice here, like using 3.9 instead of latest as template.
    ...
    ./python3-${VERSION}_post-install.sh

We´ll lock this directory down later, below.
...
Before you continue manually, try the below!
You may make a requirements file from the *ordinary* python installation

    # ml python/${VERSION}
    # which pip3
    # pip3 freeze > requirements${VERSION}.txt
    # ml python3/${VERSION}
    # which pip3
    # pip3 install -r requirements.txt

In other case follow the manual steps below
